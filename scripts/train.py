...
def load_data():
    ...
    train_data_path = os.path.join(project_dir, "data", "train_timeseries", "train_timeseries.csv")
    val_data_path = os.path.join(project_dir, "data","validation_timeseries", "validation_timeseries.csv")
    test_data_path = os.path.join(project_dir, "data","test_timeseries", "test_timeseries.csv")
...
def mlflow_workflow():
    ...
    # Preprocess data
    X_train, y_train, X_val, y_val, X_test, y_test, dv = preprocess_data(train_data, test_data, val_data)

    # Train and log model
    model_version = train_and_log_model(X_train, y_train, X_val, y_val, X_test, y_test, dv)
...
if __name__ == "__main__":
    ...
    deployment = Deployment.build_from_flow(
        flow=mlflow_workflow,
        name="Draught_prediction"
    )
    deployment.apply()
